...
public class TimedLevel : MonoBehaviour
{
    ...
    private void Start()
    {
        Time.fixedDeltaTime = 1f;//0.001f;
    }
    private void FixedUpdate()
    {
        if (TimeManager.ifTimeIs(1f, ref tasks[0]))
        {
            bulletSpawner.index = 0;
            bulletSpawner.SpawnBullets();
        }
        if (TimeManager.ifTimeIs(2f, ref tasks[1]))
        {
            bulletSpawner.index = 1;
            bulletSpawner.SpawnBullets();
        }
        if (TimeManager.ifTimeIs(3f, ref tasks[2]))
        {
            bulletSpawner.index = 2;
            bulletSpawner.SpawnBullets();
        }
        if (TimeManager.ifTimeIs(4f, ref tasks[3]))
        {
